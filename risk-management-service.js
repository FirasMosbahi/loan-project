...
async function main() {
    ...
    await consumer.subscribe({ topic: 'risk-management-service', fromBeginning: true })
    ...
    await consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
            ...
            if(id) {
                console.log(`received message from ${id} with document ${request} and score ${score}`)
                ...
                if (finalScore > 50) {
                    console.log('debt ratio is enought and borrower have not any unpaid commitment')
                    isApproved = true
                } else {
                    console.log('debt ratio is not enought or borrower have an unpaid commitment')
                    isApproved = false
                }
                ...
                await producer.send({
                    topic: 'decision',
                    messages: [
                        { value: JSON.stringify({id , request ,isApproved}) },
                    ],
                })
                ...
                await producer.send({
                    ...
                    messages: [
                        { value: JSON.stringify({id , isApproved , request}) },
                    ],
                })
